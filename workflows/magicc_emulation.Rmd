...
```{r}
# remove NAs from the the magicc emulated model result
magicc_model <- lapply(magicc_emulated_model_result, function(df) {
...
magicc_result_normalized <- lapply(magicc_model, function(df) {
...
# get names 
names(magicc_result_normalized) <- names(magicc_emulated_model_result)

## subset to only include the variable of interest
magicc_gsat_data <- lapply(magicc_result_normalized, function(df) {
...
## compute summary statistics
magicc_summary_no_weighting <- lapply(names(magicc_gsat_data), function(scenario_names) {
  
  df = magicc_gsat_data[[scenario_names]]
...
magicc_no_weighting_plot_df <- do.call(rbind, magicc_summary_no_weighting)
magicc_no_weighting_plot_df <- subset(magicc_no_weighting_plot_df, year > 1994)

magicc_parametric_uncertainty_plot <- ggplot() +
  geom_ribbon(data = magicc_no_weighting_plot_df, 
              aes(x = year, 
                  ymin = ci_10, 
                  ymax = ci_90), 
              fill = "grey", alpha = 0.5) +
  geom_ribbon(data = magicc_no_weighting_plot_df, 
              aes(x = year, 
                  ...
                  ymax = ci_67), 
              fill = 'orange', alpha = 0.5) +
    geom_line(data = magicc_no_weighting_plot_df, 
            aes(x = year, y = median), 
            color = "black") +
  facet_wrap(~scenario)
magicc_parametric_uncertainty_plot
