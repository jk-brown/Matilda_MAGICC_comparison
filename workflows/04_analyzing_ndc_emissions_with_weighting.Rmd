...
Load the `model_result` data if not already in global env. For this analysis you will want to load the `model_result_new_ecs.RDS`.
...
model_scores <- lapply(model_result, function(df) {
  ...
  ocean_uptake <- score_runs(df, criterion_ocean_uptake, score_ramp, w1 = 0.1, w2 = 0.3)
...
# NOTES
Still a lot of uncertainty in the results. This high amount of uncertainty results in differences in probabilities of warming. 

Why is it that in order to get median and bounds that are consistent with magicc, there is still a high probability of low warming? the only thing I can think of is that there is too much uncertainty in the result. This is most probably driven by the new ECS distribution being used. 

So, what happens if I try this again with the old ECS distribution using the `score_ramp()` method? Will this produce a more desirable result? or will it take altering the ECS distribution again? If there is enough uncertainty with the default ECS distribution then using score ramp on a defult Hector ensemble might work. It would just be a matter of tuning with `score_ramp()` and the percentage of runs being filtered.